...
from .collectives import all_reduce, broadcast, get_num_processes
...
import cupy
from .collectives import get_rank
class DistributedDataParallel(nn.Module):
    def __init__(self, module, device_ids=None, output_device=None, dim=0):
        ...
        for name, p in self.module.state_dict().items():
            ...
            diff = p.sub(pc).abs().max()
            print(name, diff, list(p.view(-1)[:5]))
        cupy.cuda.profiler.stop()
        print('=' * 40)
        template = list(self.module.state_dict().values())
        _flat_p = _flatten_tensors(template)
        other_flat_p = _flat_p.clone()
        if get_rank() == 0:
            broadcast(_flat_p, 0)
            broadcast(other_flat_p, 1)
        else:
            broadcast(other_flat_p, 0)
            broadcast(_flat_p, 1)
        for name, orig, other in zip(self.module.state_dict().keys(), _unflatten_tensors(_flat_p, template), _unflatten_tensors(other_flat_p, template)):
            if orig.sub(other).abs().max() > 0:
                print(name)
        # print(_flat_p.sub(other_flat_p).abs().max())
